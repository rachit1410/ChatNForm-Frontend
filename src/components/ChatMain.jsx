...
function timeAgo(dateString) {
  ...
  if (diffWeek > 0) return `${diffWeek} wk${diffWeek > 1 ? "s" : ""} ago`;
        ...
        {loading ? (
          ...
          <>
            {selectedChat ? (
              <>
                {/* Chat Header */}
                <div className="flex items-center justify-between p-4 border-b border-gray-200 bg-emerald-400 text-white z-10">
                  {" "}
                  {/* Added z-10 */}
                ...
                <div
                  id="chat-body"
                  ref={chatBodyRef}
                  className="absolute inset-0 top-[76px] bottom-[72px] p-6 overflow-y-auto flex flex-col justify-end bg-chat-pattern bg-cover bg-center"
                >
                  {/* old messages */}
                  {messages.map((msg) => (
                    <div
                      ...
                      className={`flex flex-col mb-2 ${
                        msg.sent_by.id === user.id ? "items-end" : "items-start"
                      }`}
                    >
                      <div
                        className={`p-3 rounded-lg shadow-md relative ${
                          msg.sent_by.id === user.id
                            ? "bg-green-200 text-gray-800 rounded-br-none"
                            : "bg-white text-gray-800 rounded-bl-none"
                        }`}
                        style={{ maxWidth: "75%" }} // Optional: Set a max-width for consistency
                      >
                        <p className="text-emerald-500 text-xs">
                          from: {msg.sent_by.name}
                        </p>
                        <p className="text-sm mb-1 mr-10">{msg.text_message}</p>
                    ...
                    newMessages.map((msg) => (
                      ...
                      >
                        ...
                        >
                          ...
                          <p className="text-sm mb-1 mr-10">{msg.message}</p>
                          <span className="text-xs text-gray-500 absolute bottom-1 right-2">
                ...
                <div className="p-4 border-t border-gray-200 bg-gray-100 flex items-center w-full absolute bottom-0 left-0 right-0 z-10">
                  {" "}
                  {/* Added absolute, bottom-0, left-0, right-0, z-10 */}
